# Pipeline MVP Conversational Ordering trong 24 giờ

## 1. Mục tiêu và kiến trúc

Xây dựng MVP tiếng Việt cho phép khách hàng qua Messenger hoặc web fallback:
...
Mock Menu / Loyalty / Voucher / OMS
        ↓
Response Renderer → Messenger / Web
        ↓
...
- Frontend fallback và operations: React, TypeScript, Vite.
...
- Deploy: một backend public HTTPS và một frontend tĩnh. Nếu chưa chọn cloud ở giờ thứ 4, dùng HTTPS tunnel cho webhook và giữ web chat làm phương án demo chắc chắn.
...
- `POST /api/chat`: web fallback gửi `{session_id, message}` và nhận `{message, cart, quick_replies, state}`.
...
- Người 5: dựng React web chat fallback.
- Người 6: tạo test matrix, demo script và bộ utterance tiếng Việt.
- Checkpoint giờ 2: một request chat phải gọi được ít nhất một mock tool; web chat phải gửi được message.
...
- Frontend engineer: `/chat`, cart summary, quick replies và trạng thái loading/error.
...
- Checkpoint giờ 8: web chat hoàn tất luồng tìm món → thêm giỏ → báo giá.

### Giờ 8–14: tích hợp end-to-end

- Hoàn thiện OTP, loyalty, voucher, delivery details, confirmation token và tạo đơn.
- Nối Messenger vào cùng application service mà web chat đang dùng.
- Thêm handoff summary gồm lý do, trạng thái giỏ hàng và transcript đã che PII.
- Deploy public HTTPS hoặc kích hoạt tunnel.
- Checkpoint giờ 14: web chat phải tạo được mã đơn; Messenger phải nhận và phản hồi ít nhất một hội thoại nếu Meta setup thành công.

### Giờ 14–20: đánh giá và hardening

- Chạy bộ tối thiểu 60 utterances tiếng Việt cho menu, cart, voucher, loyalty, OTP, confirmation và handoff.
...
- Tìm món bằng tiếng Việt có dấu, không dấu và tên gần đúng.
...
- Báo cáo `order_count` và `completion_rate` tách theo `messenger` và `web`.
...
- Thanh toán thật, Zalo, Mobile App, API KFC thật, production HA và App Review nằm ngoài MVP.
...
- Nếu Messenger chưa sẵn sàng trước giờ 14, demo chính chuyển sang web chat; kiến trúc và application core không thay đổi.
