...
export type MenuSearchInput = { query: string; categoryQuery?: string | null; itemType?: string | null; categoryIds?: string[]; limit?: number };
...
export interface MenuSearch { search(input: string | MenuSearchInput): Promise<MenuSearchResult[]>; searchCategories(query: string): Promise<CategorySearchResult[]>; }
...
export class PrismaMenuSearch implements MenuSearch {
  ...
  async search(inputValue: string | MenuSearchInput): Promise<MenuSearchResult[]> {
    ...
    const categoryClause = input.categoryIds?.length ? Prisma.sql`OR mic.category_id IN (${Prisma.join(input.categoryIds)})` : Prisma.empty;
